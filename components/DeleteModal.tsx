...
const DeleteModal: React.FC<ModalProps> = ({ checkHandler, deleteHandler }) => {
  return (
    <div
      className="fixed top-0 left-0 w-screen h-screen flex items-center justify-center bg-gray-500 bg-opacity-50"
      onClick={(e) => {
        checkHandler(e);
      }}
    >
      <div className="inline-flex flex-col p-10 text-sm sm:text-base md:text-lg lg:text-xl border-2 border-gray-box bg-white rounded-lg ">
        ...
        <div className="flex justify-center items-center ">
          ...
          >
            <Button bg="border-2 border-gray-box" px="px-5" textSize="text-xs sm:text-xs md:text-sm lg:text-base" textColor="text-black">
              돌아가기
            </Button>
          </div>
          <span className="ml-4 ">
